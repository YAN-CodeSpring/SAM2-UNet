...
class SAM2UNet(nn.Module):
    # ========== 修改1：新增num_classes参数，默认1（兼容原有BUSI） ==========
...
if __name__ == "__main__":
    with torch.no_grad():
        # ========== 测试1：BUSI（默认1类） ==========
        model_busi = SAM2UNet().cuda()
        x = torch.randn(1, 3, 352, 352).cuda()
        out, out1, out2 = model_busi(x)
        print("BUSI输出形状（1类）：", out.shape, out1.shape, out2.shape)  # 应输出 (1,1,352,352)
        
        # ========== 测试2：BCSS（22类） ==========
        model_bcss = SAM2UNet(num_classes=22).cuda()
        x_bcss = torch.randn(1, 3, 224, 224).cuda()
        out_bcss, out1_bcss, out2_bcss = model_bcss(x_bcss)
        print("BCSS输出形状（22类）：", out_bcss.shape, out1_bcss.shape, out2_bcss.shape)  # 应输出 (1,22,224,224)