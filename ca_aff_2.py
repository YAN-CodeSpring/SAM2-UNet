...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        # 2. 模型移到GPU
        model = SAM2UNet().to(device)  # 关键：加.to(device)
        
        # 3. 输入张量移到同一个设备
        x = torch.randn(2, 3, 224, 224).to(device)  # 关键：加.to(device)
        
        # 4. 前向传播（此时模型和输入都在GPU，类型匹配）
        out, out1, out2 = model(x)  # 现在能正常执行到这步，看到x1的shape了
